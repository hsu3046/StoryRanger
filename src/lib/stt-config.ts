...
/** Server-side upload gate. 6 s of AAC is ~190 KB, opus ~70 KB. This must
 *  stay TIGHT because it is the real duration cap — the 6 s cut above is
 *  client-side and not trusted: at 1 MB an opus clip could smuggle ~80 s of
 *  audio per request, ~13× the per-day cost the request counter assumes.
 *  300 KB bounds opus at ~25 s worst-case while clearing real 6 s clips. */
export const MAX_AUDIO_BYTES = 300_000;
